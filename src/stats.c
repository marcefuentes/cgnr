		  ...
		  int n)
{
	...
	int correlationPairs[CORRELATIONS][2] = {
		{ 2, 3 }, { 2, 4 }, { 2, 5 }, { 2, 6 }, { 2, 7 },
		{ 3, 4 }, { 3, 5 }, { 3, 6 }, { 3, 7 }, { 4, 5 },
		{ 4, 6 }, { 4, 7 }, { 5, 6 }, { 5, 7 }, { 6, 7 }
	};
	...
	for (; i < i_last; i++) {
		double *properties[CONTINUOUS_V] = {
			&i->w,		 &i->qBDefault,	     &i->qBSeen,
...
void stats_runs(struct ptype *p, struct ptype *p_last, int runs)
{
	for (; p < p_last; p++) {
		for (int v = 0; v < CONTINUOUS_V; v++) {
			for (int b = 0; b < BINS; b++) {
				p->c2[v][b] =
					stdev(p->c[v][b], p->c2[v][b], runs);
				p->c[v][b] /= runs;
			}

			p->median2[v] =
				stdev(p->median[v], p->median2[v], runs);
			p->median[v] /= runs;
