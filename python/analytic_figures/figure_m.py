...
def update(t, mode, df_rows, dffrq_rows, movie, text, artists):
    """ Update the plot with the data at time t. """

    if dffrq_rows:
        alphas = np.sort(df_rows["none"]["alpha"].unique())[::-1]
        logess = np.sort(df_rows["none"]["logES"].unique())
    traits = mm.get_traits(mode)
    for r, row in enumerate(df_rows):
        kws = ["cooperation", "correlations", "test"]
        if any(kw in mode for kw in kws) and row == "social":
            continue
        for c, trait in enumerate(traits):
            zmatrix = update_zmatrix(t, df_rows, row, trait, mode)
            if dffrq_rows:
...
def main(mode, histogram=False, movie=False):
    ...
    for row in rows:
        df_rows[row] = get_df(row, "csv", movie)
        if histogram:
            dffrq_rows[row] = get_df(row, "frq", movie)
    if "social" not in rows:
        df_rows["social"] = get_df("social", "csv", movie)
    df = df_rows[rows[0]]
    ts = df.Time.unique()
    nr = df.alpha.nunique()
    nc = df.logES.nunique()
