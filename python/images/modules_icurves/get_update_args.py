...
def get_update_args(update_args):
    ...
    update_args["alphas"] = np.linspace(
        get_config("alpha_max"), get_config("alpha_min"), num=get(update_args["file_name"], "nc")
    )
    update_args["logess"] = np.linspace(
        get_config("loges_min"), get_config("loges_max"), num=get(update_args["file_name"], "nc")
    )
    update_args["rhos"] = 1.0 - 1.0 / np.power(2.0, update_args["logess"])
    update_args["icx"] = np.linspace(0.001, 0.999, num=get(update_args["file_name"], "n_x_values"))
    update_args["budget_0"] = 1.0 - update_args["icx"]

    w_isoclines = np.linspace(
        1.0 / (get(update_args["file_name"], "n_ic") + 1),
        get(update_args["file_name"], "n_ic") / (get(update_args["file_name"], "n_ic") + 1),
