...
import figures.modules_theory.theory as tt
...
axs = np.empty((2, len(alphas), len(rhos)), dtype=object)

fig = plt.figure(figsize=(width, height))
outergrid = fig.add_gridspec(
    nrows=1, ncols=2, left=0.15, right=0.85, top=0.8, bottom=0.2
)
for g in range(2):
    grid = outergrid[g].subgridspec(nrows=num, ncols=num, wspace=0, hspace=0)
    axs[g] = grid.subplots()
...
fig.supxlabel(xlabel, x=center_x, y=bottom_y * 0.3, fontsize=biglabel)
fig.supylabel(ylabel, x=left_x * 0.4, y=center_y, fontsize=biglabel)
...
for g in range(2):
    letter = ord("a") + g
    axs[g, 0, 0].set_title(
        chr(letter), fontsize=plotsize * 5, weight="bold", loc="left"
    )
    if g == 0:
        for a in range(0, num, step):
            axs[g, a, 0].set_ylabel(
                f"{alphas[a]:.1f}",
                rotation="horizontal",
    ...
    for r in range(0, num, step):
        axs[g, -1, r].set_xlabel(f"{logess[r]:.0f}", fontsize=ticklabel)
...
for g in range(2):
    for a, alpha in enumerate(alphas):
        for r, rho in enumerate(rhos):
            ...
            (budgets[g, a, r],) = axs[g, a, r].plot(
                budgetx, dummy_budgety, c="0.300", linewidth=4, alpha=0.8
            )
            (icurves[g, a, r],) = axs[g, a, r].plot(
                icx, dummy_icy, linewidth=4, alpha=0.8
            )
...
cbar = fig.colorbar(
    cm.ScalarMappable(norm=norm, cmap="Reds"), cax=axins, ticks=[0, 0.5, 1]
)
